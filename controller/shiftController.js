...
async function getShift(Id) {
    try {
        ...
        return shiftRecords.recordset[0] || null;
    } catch (error) {
        console.error(error);
...
async function addShift(shift) {
    try {
        ...
        let insertShift = await pool.request()
            ...
            .query(`
                INSERT INTO Shift (WorkType, ShiftName, ShiftFrom, ShiftTo)
                OUTPUT INSERTED.Id
                VALUES (@WorkType, @ShiftName, @ShiftFrom, @ShiftTo)
            `);
...
async function updateShift(Id, shift) {
    try {
        ...
        let updateShift = await pool.request()
            ...
            .query(`
                UPDATE Shift
                SET WorkType = @WorkType, ShiftName = @ShiftName, ShiftFrom = @ShiftFrom, ShiftTo = @ShiftTo
                WHERE Id = @Id
            `);
