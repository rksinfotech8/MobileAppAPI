...
async function addWorker(worker) {
    try {
        ...
        let insertWorker = await pool.request()
            ...
            .query("INSERT INTO Worker (FirstName, LastName, MobileNumber, Work, WorkType) VALUES (@FirstName, @LastName, @MobileNumber, @Work, @WorkType)");
...
async function updateWorker(Id, worker) {
    try {
        ...
        let updateWorker = await pool.request()
            ...
            .input('Work', sql.VarChar, worker.Work)
            .input('WorkType', sql.VarChar, worker.WorkType)
            .query("UPDATE Worker SET FirstName = @FirstName, LastName = @LastName, MobileNumber = @MobileNumber, Work = @Work, WorkType = @WorkType WHERE Id = @Id");
